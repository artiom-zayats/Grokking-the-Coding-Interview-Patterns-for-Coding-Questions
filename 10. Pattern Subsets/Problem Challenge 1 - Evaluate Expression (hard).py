...
def diff_ways_to_evaluate_expression(input):
  ...
  for i in range(len(input)):
    if input[i] in ["*","+","-"]:
      res1 = diff_ways_to_evaluate_expression(input[:i])
      res2 = diff_ways_to_evaluate_expression(input[i+1:])
      if input[i] == "+":
        temp = [int(x)+int(y) for x in res1 for y in res2]
      if input[i] == "*":
        temp = [int(x)*int(y) for x in res1 for y in res2]
      if input[i] == "-":
        temp = [int(x)-int(y) for x in res1 for y in res2]
