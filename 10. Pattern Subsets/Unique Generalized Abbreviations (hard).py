...
def generate_generalized_abbreviation(word):
    result = []
    n = len(word)
    def dfs(ix,abb,count):
      #print(ix,abb,count)
      if ix == n:
        if count != 0:
          abb.append(str(count))
        result.append("".join(abb))
        return

      #use number
      dfs(ix + 1, list(abb), count + 1)

      #if there is number add this number to string
      if count != 0:
        abb.append(str(count))
      #count with letter
      dfs(ix+1,abb + [word[ix]],0)
...
def main():
    print("Generalized abbreviation are: " +
          str(generate_generalized_abbreviation("BAT")))
    print("Generalized abbreviation are: ABA " +
          str(generate_generalized_abbreviation("ABA")))
    print("Generalized abbreviation are: " +
          str(generate_generalized_abbreviation("code")))
